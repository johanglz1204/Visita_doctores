...
async function generatePurchaseOrder(data, templatePath) {
    ...
    data.forEach((item, index) => {
        ...
        if (codeStr.startsWith('0')) {
            // Mantener como texto para no perder el cero a la izquierda
            sheet.cell(currentRow, 1).value(codeStr);
        } else {
            // Convertir a número JS real y forzar formato General para evitar advertencias de Excel
            const numVal = Number(codeStr);
            sheet.cell(currentRow, 1).value(isNaN(numVal) ? codeStr : numVal).style('numberFormat', 'General');
        }
    ...
    const outputName = `Pedido_Semanal_${fecha}.xlsx`;
    const outputPath = path.join(path.dirname(templatePath), outputName);

    console.log('💾 Guardando archivo intermedio...');
    await workbook.toFileAsync(outputPath);

    // --- POST-PROCESAMIENTO: Forzar recálculo automático de fórmulas ---
    // xlsx-populate no recalcula las fórmulas y borra el valor cacheado (<v>).
    // Si el usuario abre el archivo en "Vista Protegida", las celdas de fórmulas
    // se verán vacías hasta que Excel recalcule todo. Añadimos fullCalcOnLoad="1"
    // para forzar a Excel a recalcular apenas se habilite la edición.
    console.log('🔧 Post-procesando: forzando recálculo automático de fórmulas...');
    const fileData = fs.readFileSync(outputPath);
    const zip = await JSZip.loadAsync(fileData);
    let workbookXml = await zip.files['xl/workbook.xml'].async('string');
    
    // Buscar la etiqueta calcPr y añadirle fullCalcOnLoad="1" si no lo tiene
    if (workbookXml.includes('<calcPr ') && !workbookXml.includes('fullCalcOnLoad="1"')) {
