  ...
  try {
    ...
    const [
      ...
      knex('inventory_stocks')
        .whereRaw('current_stock <= 2 OR (target_stock > 0 AND (CAST(current_stock AS NUMERIC) / target_stock) <= 0.2)')
        .count('* as count'),
    ...
    const [
      ...
      knex('sales_history')
        .select(knex.raw("strftime('%Y-%m-%d', sale_date) as date"))
        .select(knex.raw("CAST(SUM(quantity) AS INTEGER) as total_quantity"))
      ...
      knex('doctors as d')
        ...
        .select(knex.raw("CAST(julianday('now') - julianday(MAX(s.sale_date)) AS INTEGER) as inactive_days"))
        .groupBy('d.id', 'd.name', 'd.phone')
        .having(knex.raw("CAST(julianday('now') - julianday(MAX(s.sale_date)) AS INTEGER) >= 30"))
      ...
      knex('sales_history')
        ...
        .select(knex.raw("CAST(SUM(quantity) AS INTEGER) as value"))
        .where('sale_date', '>=', thirtyDaysAgoStr)
        .groupBy('sucursal')
        .orderBy('value', 'desc'),
      ...
      knex('sales_history as s')
        ...
        .select(knex.raw("substr(p.name, 1, instr(p.name || ' ', ' ') - 1) as line"))
        .select(knex.raw("CAST(SUM(s.quantity) AS INTEGER) as value"))
        .where('s.sale_date', '>=', thirtyDaysAgoStr)
        .groupByRaw("substr(p.name, 1, instr(p.name || ' ', ' ') - 1)")
        .orderBy('value', 'desc')
        .limit(8),
