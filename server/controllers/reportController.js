...
const generateExecutiveReport = async (req, res, next) => {
  try {
    ...
    doc.fontSize(10).fillColor('#c4b5fd').font('Helvetica')
      .text(`SICOIN — Periodo: ${days} días (${startStr} a ${new Date().toISOString().split('T')[0]})`, 50, 55);
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      doc.fontSize(7).fillColor(COLORS.gray).font('Helvetica')
        .text(
          `Generado el ${new Date().toLocaleString('es-MX')} | SICOIN | Página ${i + 1} de ${pages.count}`,
