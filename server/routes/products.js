...
router.get('/', async (req, res) => {
  try {
    ...
    const formatted = rows.map(r => ({
      ...r,
      stock_by_branch: typeof r.stock_by_branch === 'string' ? JSON.parse(r.stock_by_branch) : r.stock_by_branch
    }));
    res.json(formatted);
  } catch (err) {
...
router.put('/:id', validateRequest(productSchema), async (req, res) => {
  try {
    const { name, barcode, ranking, price } = req.body;
    const { rows } = await db.query(
      `UPDATE products SET name=$1, barcode=$2, ranking=$3, price=$4, updated_at=CURRENT_TIMESTAMP
