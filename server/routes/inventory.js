  ...
  try {
    ...
    const { rows } = await db.query(`
      SELECT 
        ...
        i.current_stock,
        CASE 
          WHEN i.target_stock > 0 THEN ROUND((CAST(i.current_stock AS NUMERIC) / i.target_stock) * 100)
          ELSE 0 
      ...
      WHERE i.current_stock <= $1 
         OR (i.target_stock > 0 AND (CAST(i.current_stock AS NUMERIC) / i.target_stock) <= 0.2)
  ...
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO inventory_stocks (doctor_id, product_id, target_stock, current_stock)
       ...
       DO UPDATE SET target_stock = $3, current_stock = $4, updated_at = CURRENT_TIMESTAMP
       RETURNING *`,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `UPDATE inventory_stocks SET target_stock=$1, current_stock=$2, updated_at=CURRENT_TIMESTAMP
