...
router.get('/status', authenticate, async (req, res) => {
  try {
    ...
    if (lastSync.length > 0) {
      const { rows: detail } = await db.query('SELECT unmatched_list, matched_list FROM mysql_sync_logs WHERE id = $1', [lastSync[0].id]);
      try {
        unmatchedList = typeof detail[0]?.unmatched_list === 'string' ? JSON.parse(detail[0].unmatched_list) : (detail[0]?.unmatched_list || []);
        matchedList = typeof detail[0]?.matched_list === 'string' ? JSON.parse(detail[0].matched_list) : (detail[0]?.matched_list || []);
      } catch (e) {
        console.error('Error parsing sync lists:', e);
      }
    }
...
router.post('/map', authenticate, async (req, res) => {
  try {
    ...
    // Insertar o actualizar el alias (SQLite: INSERT OR REPLACE)
    await db.query(`
      INSERT INTO product_aliases (alias_name, product_id)
      VALUES ($1, $2)
      ON CONFLICT(alias_name) DO UPDATE SET product_id = excluded.product_id, created_at = CURRENT_TIMESTAMP
    `, [alias_name.trim().toLowerCase(), product_id]);
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT id, name, barcode, ranking, stock 
       FROM products 
       WHERE LOWER(name) LIKE $1 OR barcode LIKE $2
       ORDER BY name ASC
       LIMIT 15`,
...
router.post('/generate-order', authenticate, async (req, res) => {
  ...
  syncInProgress = true;
  const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'generate_order_local.js');
  
  console.log(`🚀 [Excel Gen] Ejecutando script local (sin Firebase): ${scriptPath}`);
  ...
  exec(`node --max-old-space-size=4096 "${scriptPath}"`, (error, stdout, stderr) => {
    ...
    // Buscamos el nombre del archivo generado en el stdout (✅ Pedido generado exitosamente: Pedido_Semanal_YYYYMMDD.xlsx)
    const match = stdout.match(/Pedido_Semanal_\d+\.xlsx/);
