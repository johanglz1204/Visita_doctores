  ...
  return [
    {
      ...
      password: process.env.APP_PASSWORD || '7294967290',
    }
  ];
}
...
router.post('/login', loginLimit, (req, res) => {
  ...
  // Access Token: 8 hours (desktop app, no need to expire every 15 min)
  const accessToken = jwt.sign({ username: user.username }, JWT_SECRET, { expiresIn: '8h' });
